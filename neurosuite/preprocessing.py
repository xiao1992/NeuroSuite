...
class Preprocessor:
    ...
    def _infer_channel_names(self, n_channels):
        return [f"EEG {i+1:03}" for i in range(n_channels)]
    ...
    def apply_ica_mne(self, data):
        ...
        ch_names = self.ch_names or self._infer_channel_names(n_channels)

        if isinstance(self.ch_types, str):
            ch_types = [self.ch_types] * n_channels
        else:
    ...
    def transform(self, X):
        ...
        for trial in X:
            ...
            if self.apply_filter and not self.apply_ica: 
